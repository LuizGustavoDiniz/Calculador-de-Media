...
averageCalculatorForm.addEventListener('submit', event => {
  ...
  const grades = Array.from(averageCalculatorForm.nota)

  const gradesValue = grades.map(grade => Number(grade.value)).reduce((acc, grade) => acc + grade, 0) / 4
  ...
  res.innerHTML = `
   ...
   <p>Media de ${studentName} : ${Math.round(gradesValue)}</p>
  `
})
